...
if (mikro == false){
	float w = exp(-dE / (temperatura_docelowa_ukladu));
	float r_ = generator.losuj_z_zakresu_0_1();

	// Jeśli dE <= 0 lub potem r < w to akceptacja nowego stanu
	if (r_ <= w || dE <= 0)
	{
		siatka[i][j] = -siatka[i][j];
        energia_początkowa_układu  += dE;
